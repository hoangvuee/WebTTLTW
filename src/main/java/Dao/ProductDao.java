...
String temp = checkState(isActive)  ;
    ...
    public boolean updateProduct( int id, String productName, int idCategory, int idSupplier, String imagePath) {
        String sql = "UPDATE products SET productName = ?, idCategory = ?, idSupplier = ?,  image = ? WHERE id = ?";
        ConnDB conn = new ConnDB();

        try (PreparedStatement stmt = conn.getConn().prepareStatement(sql)) {
            stmt.setString(1, productName);
            stmt.setInt(2, idCategory);
            stmt.setInt(3, idSupplier);

            stmt.setString(4, imagePath); // có thể là tên file hoặc đường dẫn tương đối
            stmt.setInt(5, id);
    ...
    public boolean updateProduct( int id, String productName, int idCategory, int idSupplier) {
        String sql = "UPDATE products SET productName = ?, idCategory = ?, idSupplier = ? WHERE id = ?";
        ConnDB conn = new ConnDB();
    ...
    public boolean updateProductDetail( int id, int idProduct, int weight, int price, boolean isActive, int quantity) {
        String sql = "UPDATE product_variants SET idProduct = ?, weight = ?, price = ?, isActive = ? ,quantity = ? WHERE id = ?";
        ConnDB conn = new ConnDB();
